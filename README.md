...
`glurp` is a Linux/macOS CLI that privately mirrors AI coding-chat archives from remote machines over OpenSSH. It collects Claude Code, Codex, OpenCode, and pi histories without parsing or normalizing transcript content.
...
glurp host list
glurp host list --json
glurp host remove lab
```

Names contain lowercase letters, digits, dots, underscores, or dashes. A destination is one argument and cannot contain options or whitespace; put complex connection behavior in `~/.ssh/config`. Glurp never stores passwords, keys, agents, or known-host data.
...
Config and archive overrides must have a trusted parent chain. Glurp rejects non-sticky directories writable by other accounts; a sticky shared parent such as `/tmp` is permitted, while Glurp-owned descendants are tightened to owner-only modes.
...
This repository and command are a clean rename from `slurp`; there is no legacy command, configuration, archive migration, or compatibility alias. Slurp state is not automatically used, migrated, or deleted by Glurp.
